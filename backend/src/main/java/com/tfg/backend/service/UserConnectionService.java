...
public class UserConnectionService {
    ...
    public void enrichWithConnections(List<UserDTO> users) {
        ...
        List<UserConnection> connections = connectionRepository.findAllById(usernames);

        Map<String, UserConnection> connMap = connections.stream()
                .collect(Collectors.toMap(UserConnection::getUsername, c -> c));

        for (UserDTO user : users) {
            UserConnection conn = connMap.get(user.getUsername());
